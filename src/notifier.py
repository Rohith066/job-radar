...
def _build_html(yes_jobs: list[Job], maybe_jobs: list[Job], mode: str, source_errors: list[str] | None = None) -> str:
    ...
    def _card(job: Job) -> str:
        ...
        # Visa sponsorship banner — critical for H1B
        visa_status = getattr(job, "visa_status", "")
        if visa_status == "none":
            ghost_banner += (
                '<div class="ghost-suspicious">&#128679; <strong>No visa sponsorship</strong> — '
                'JD states sponsorship is not available. Likely not viable on H1B.</div>'
            )
        elif visa_status == "available":
            ghost_banner += (
                ...
                '&#127757; <strong>Visa sponsorship available</strong></div>'
            )
