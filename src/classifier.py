...
# Tier-2 titles that belong to the AI/ML track rather than the DE track.
# Checked on word boundaries so "ml" doesn't match inside other words.
_TIER2_AI_HINTS = (
    "ml", "machine learning", "mlops", "ai", "applied scientist",
    "deep learning", "nlp",
)


def _tier2_track(t: str) -> str:
    """Assign a resume track to a Tier-2 title.

    ML/AI-flavoured roles route to the AI resume; analyst/BI/science roles to
    the analyst resume; everything else defaults to the DE resume.
    """
    if any(re.search(rf"(?<![a-z]){re.escape(h)}(?![a-z])", t) for h in _TIER2_AI_HINTS):
        return "ai"
    # Engineer-flavoured titles take the DE resume even when they carry a BI
    # label — "BI Engineer" and "Business Intelligence Engineer" are the same
    # role and must not diverge.
    if "engineer" in t:
        return "de"
    if any(w in t for w in ("analyst", "scientist", "developer", "bi ")):
        return "analyst"
    return "de"
