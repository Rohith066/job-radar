...
from .matching.config import AUTO_INTERESTED_THRESHOLD
from .utils.visa import detect_visa_sponsorship, is_work_authorization_blocked
...
def _dedup_jobs(jobs: list[Job]) -> list[Job]:
    """Remove duplicates using a deterministic fingerprint hash of
    company|title|city. Catches the same role posted across multiple sources
    (e.g. company careers page + Remotive). On collision, keeps the record with
    the higher score, preferring the one that already has a JD description."""
    best: dict[str, Job] = {}
    for j in jobs:
        fp = job_fingerprint(j.company, j.title, j.location)
        existing = best.get(fp)
        if existing is None:
            best[fp] = j
            continue
        # Prefer higher score; tie-break toward the job that has a description
        j_rank = (j.score, 1 if j.description else 0)
        e_rank = (existing.score, 1 if existing.description else 0)
        if j_rank > e_rank:
            best[fp] = j
...
def run_main(cfg: Config, db: Database, notifier: CompositeNotifier, *, dry_run: bool, no_notify: bool, test_notify: bool) -> None:
    ...
    # Free public-API sources — remote + US jobs with full JD descriptions
    if cfg.source("remotive").enabled:
        sources.append(RemotiveSource(max_jobs=cfg.source("remotive").max_jobs))
    if cfg.source("remoteok").enabled:
        sources.append(RemoteOKSource(max_jobs=cfg.source("remoteok").max_jobs))
    if cfg.source("themuse").enabled:
        sources.append(TheMuseSource(max_jobs=cfg.source("themuse").max_jobs))
...
def _boards_cursor_key(csv_path: str) -> str:
    """Per-CSV cursor name so sharded workflows sweeping different board
    files don't clobber each other's sweep position.

    The original SUPPORTED csv keeps the historical 'boards_main' key so
    its in-flight cursor survives this change.
    """
    stem = os.path.splitext(os.path.basename(csv_path))[0].lower()
    if stem == "job_boards_pure_working_supported_round2":
        return "boards_main"
    return "boards_" + re.sub(r"[^a-z0-9]+", "_", stem).strip("_")
...
def run_boards(
    ...
    *,
    ...
    cursor_key = _boards_cursor_key(boards_csv)
    n = len(boards)
...
def _dispatch_results(
    *,
    ...
    # "available" → +5 boost; "none" → −20 penalty (down-rank, still shown with warning)
    visa_none = visa_yes = 0
    for j in matched:
        status, phrase = detect_visa_sponsorship(j.description or "")
        j.visa_status = status
        j.visa_phrase = phrase
        if status == "available":
            j.score = min(100, j.score + 5)
            visa_yes += 1
        elif status == "none":
            j.score = max(0, j.score - 20)
            visa_none += 1
        if status:
            j.label = "yes" if j.score >= 70 else "maybe" if j.score >= 40 else "no"
    if visa_none or visa_yes:
        log.info("Visa filter: %d offer sponsorship (+5), %d refuse sponsorship (-20)",
                 visa_yes, visa_none)

    # Work-authorization hard filter — drop roles closed to a sponsorship-needing
    # candidate. Catches citizenship/permanent-residency/ITAR gates and security
    # clearance, not just explicit "no sponsorship" language. Citizenship is the
    # one that matters most: a role can require zero clearance and still be closed.
    before_auth = len(matched)
    kept = []
    for j in matched:
        blocked, reason = is_work_authorization_blocked(j.description or "")
        if blocked:
            log.debug("Work-auth filter dropped: %s — %s (%s)", j.company, j.title, reason)
            continue
        kept.append(j)
    auth_dropped = before_auth - len(kept)
    if auth_dropped:
        log.info("Work-auth filter: dropped %d job(s) requiring citizenship, clearance, "
                 "or no-sponsorship", auth_dropped)
    matched = kept
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    try:
        ...
        if getattr(args, "tailor", None):
            from .tailor import run_tailor
            run_tailor(db, args.tailor, write_file=getattr(args, "tailor_write", False))
            return
